...
Для розгортання бази даних використовується повний SQL-скрипт, що знаходиться у файлі `database/setup.sql`. Цей скрипт автоматично створить базу даних `music_player`, необхідні таблиці та наповнить їх тестовими даними.

**Порядок дій:**

1.  Відкрийте ваш MySQL клієнт (MySQL Workbench, DBeaver) або термінал.
2.  Увійдіть під користувачем `root` (або іншим, що має права на створення баз даних).
3.  Відкрийте та виконайте файл `database/setup.sql`.

**Альтернативно через командний рядок:**
```bash
